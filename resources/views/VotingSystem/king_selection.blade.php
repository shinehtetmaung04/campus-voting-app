...
<div class="contest-card second">
  <div class="slider">
    @php $kingImages = $king->images->take(3); @endphp
    @foreach ($kingImages as $image)
      <a href="{{ url('king/'.$king->sel_id.'?img='.$image->img_id) }}">
        <img src="{{ asset($image->image_url) }}" alt="{{ $king->name }}">
      </a>
    @endforeach
  ...
  <div class="card-body" style="height:auto;">
    <div class="name-pair">
      <h3 class="left-name">{{ $king->name }}</h3>
...
document.querySelectorAll('.vote-btn').forEach(btn => {
  btn.onclick = function() {
    ...
    pendingKingName = this.dataset.kingName;
    if(this.dataset.loggedIn === "1") {
      showConfirm(pendingKingName);
    } else {
